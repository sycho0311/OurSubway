...
if (!ismasterLogin) {
%>
	<form name=deleteform method=post action="/OurSubway/contact/delete_ok.jsp?num=<%= num %>">
	<table>
		<tr>
			<td></td>
...
<%
		String sql = "SELECT password FROM contact WHERE num=" + num;
		ResultSet res = statement.executeQuery(sql);
		if (res.next()) {
			sql = "DELETE FROM contact WHERE num=" + num;
			statement.executeUpdate(sql);
%>
			<script>
				self.window.alert("글이 삭제되었습니다");
				location.href="/OurSubway/index.jsp?main=contact";
			</script>
<%
			res.close();
			statement.close();
